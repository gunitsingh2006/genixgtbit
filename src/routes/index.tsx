...
import { ArrowRight, Sparkles, Users, Trophy, Rocket, Instagram, Calendar } from "lucide-react";
import { Reveal } from "@/components/site/Reveal";
import logo from "@/assets/genix-icon.asset.json";
import event1 from "@/assets/event1.asset.json";
import event2 from "@/assets/event2.asset.json";
import event3 from "@/assets/event3.asset.json";
import gunit from "@/assets/gunit.asset.json";
import gurnoor from "@/assets/gurnoor.asset.json";
import jasnoor from "@/assets/jasnoor.asset.json";
import agamjot from "@/assets/agamjot.asset.json";

export const Route = createFileRoute("/")({
  component: Home,
  head: () => ({
    meta: [
      { title: "Genix — GTBIT's Tech Society" },
      { name: "description", content: "Join Genix, the official tech society of GTBIT. Workshops, tech debates, hackathons — where first-years become future technologists." },
    ],
  }),
});
  ...
  return (
    <div>
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden">
        ...
        <div className="absolute -top-40 -right-40 h-[600px] w-[600px] rounded-full bg-teal/20 blur-[120px]" />
        <div className="absolute bottom-0 -left-40 h-[500px] w-[500px] rounded-full bg-accent/15 blur-[120px]" />
        ...
        <div className="relative max-w-7xl mx-auto px-5 md:px-8 py-24 grid lg:grid-cols-[1.15fr_1fr] gap-14 items-center w-full">
          ...
          <Reveal delay={0.2} className="relative hidden lg:block">
            ...
            >
              <div className="absolute inset-0 rounded-[2rem] gradient-brand opacity-30 blur-3xl" />
              <div className="relative h-full w-full rounded-[2rem] border border-border bg-card/60 backdrop-blur-xl p-10 flex items-center justify-center glow-teal">
                <img src={logo.url} alt="Genix" className="w-full h-full object-contain drop-shadow-2xl" />
              </div>
            </motion.div>
          </Reveal>
        </div>
        ...
        <div className="relative rounded-3xl overflow-hidden border border-border bg-card/60">
          ...
          <div className="grid lg:grid-cols-2 gap-0">
            ...
            <div className="relative grid grid-cols-2 gap-2 p-4 lg:p-6">
              <img src={event1.url} alt="Tech Debate participants" className="rounded-xl object-cover h-full w-full aspect-[4/5] col-span-2" loading="lazy" />
              <img src={event2.url} alt="Tech Debate group" className="rounded-xl object-cover h-full w-full aspect-square" loading="lazy" />
              <img src={event3.url} alt="Tech Debate faculty" className="rounded-xl object-cover h-full w-full aspect-square" loading="lazy" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-5 md:px-8 py-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { name: "Gunit Singh", role: "President", img: gunit.url },
            { name: "Gurnoor Singh", role: "Vice President", img: gurnoor.url },
            { name: "Jasnoor Kaur", role: "Head of Operations", img: jasnoor.url },
            { name: "Agamjot Singh", role: "Executive Director", img: agamjot.url },
          ].map((m, i) => (
      ...
      <section className="max-w-7xl mx-auto px-5 md:px-8 py-16">
        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-3">
          {[event1.url, event2.url, event3.url, gunit.url].map((src, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <a href="https://www.instagram.com/genix.gtbit" target="_blank" rel="noreferrer" className="group relative block aspect-square rounded-xl overflow-hidden border border-border">
                <img src={src} alt="Instagram post" className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" />
        ...
        <div className="mt-8 text-center">
          <a href="https://www.instagram.com/genix.gtbit" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-5 py-3 rounded-full border border-border hover:border-teal hover:text-teal transition font-semibold">
